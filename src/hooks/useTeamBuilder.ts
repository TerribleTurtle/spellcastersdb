...
export function useTeamBuilder(availableUnits: (Unit | Spell | Titan)[], availableSpellcasters: Spellcaster[]) {
  ...
  useEffect(() => {
    ...
    if (storedMeta) {
        ...
        } catch (e) { console.error("Failed to load team meta", e); }
    }
    ...
    if (storedSaved) {
        try {
            ...
            const reconstructedList = list.map(t => ({
                id: t.id,
                name: t.name,
                decks: t.deckData.map(d => reconstructDeck(d, availableUnits, availableSpellcasters)) as [Deck, Deck, Deck]
            }));
            setTimeout(() => setSavedTeams(reconstructedList), 0);
        } catch (e) { console.error("Failed to load saved teams", e); }
    }
  ...
  useEffect(() => {
      ...
      return () => window.removeEventListener('spellcasters:deck-written', handleDeckWrite);
  }, [refreshTeamDecks]);
  ...
  useEffect(() => {
      if (!hasHydrated.current) return;
      localStorage.setItem(STORAGE_KEY_TEAM_METADATA, JSON.stringify({ 
          name: teamName,
          id: activeTeamId,
          activeSlot 
      }));
  ...
  useEffect(() => {
    ...
    const storedList: StoredTeam[] = savedTeams.map(t => ({
        id: t.id,
        name: t.name,
        deckData: t.decks.map(serializeDeck) as [StoredDeck, StoredDeck, StoredDeck]
    }));
  ...
  const saveTeam = (nameInput: string, activeDeckOverride?: Deck) => {
      ...
      setSavedTeams(prev => {
          ...
          if (existingIndex >= 0) {
              const updated = [...prev];
              updated[existingIndex] = { ...updated[existingIndex], name, decks: currentDecks };
              return updated;
          } else {
      ...
      if (target) {
          ...
          target.decks.forEach((deck, idx) => {
               ...
               if (typeof window !== 'undefined') {
                   window.dispatchEvent(new CustomEvent('spellcasters:current-deck-update', { 
                       detail: { key } 
                   }));
               }
          });
  ...
  const deleteTeam = (teamId: string) => {
      ...
      if (teamDecks && savedTeams.find(t => t.id === teamId)?.name === teamName) {
  ...
  const importSoloDeck = (slotIndex: number, deck: Deck) => {
      ...
      const importDeck: Deck = { 
          ...deck, 
          id: uuidv4(),
          slots: deck.slots.map(s => ({...s})) as [DeckSlot, DeckSlot, DeckSlot, DeckSlot, DeckSlot]
      };
  ...
  const exportSlotToSolo = (slotIndex: number, decksState: Deck) => {
     const deckToExport = { ...decksState, id: uuidv4(), name: `${decksState.name} (From Team)` };
  ...
  const clearTeam = () => {
      ...
      // Create empty state
      const emptyState = TEAM_SLOT_KEYS.map(() => reconstructDeck({
            spellcasterId: null,
            slotIds: [null, null, null, null, null]
       }, availableUnits, availableSpellcasters)) as [Deck, Deck, Deck];
  ...
  const hasChanges = useMemo(() => {
      ...
      if (!activeTeamId) {
          ...
          const areDecksEmpty = teamDecks?.every(d => !d.spellcaster && d.slots.every(s => !s.unit));
