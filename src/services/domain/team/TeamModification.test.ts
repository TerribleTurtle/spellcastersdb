...
import { INITIAL_DECK } from '@/services/api/persistence'; // Assuming this exists or I need to mock it
...
describe('TeamModification', () => {
    ...
    describe('setSpellcaster', () => {
        it('should set spellcaster', () => {
             const result = TeamModification.setSpellcaster(teamDecks, 1, MockSpellcaster);
             expect(result.success).toBe(true);
             expect(result.data![1].spellcaster).toEqual(MockSpellcaster);
        });

        it('should return error for invalid deck index', () => {
             const result = TeamModification.setSpellcaster(teamDecks, 5, MockSpellcaster);
             expect(result.success).toBe(false);
             expect(result.error).toBe(TEAM_ERRORS.INVALID_DECK_INDEX);
        });
    });
    ...
    describe('swapSlots', () => {
        it('should swap two units in the same deck', () => {
             ...
             currentDecks = TeamModification.setSlot(currentDecks, 0, 0, MockUnit).data!;
    ...
    describe('quickAdd', () => {
        ...
        it('should auto-add titan to titan slot', () => {
             ...
             const hasTitan = result.data![0].slots.some(s => s.unit?.entity_id === MockTitan.entity_id);
    ...
    describe('importDeck', () => {
         it('should import a full deck into a slot', () => {
              ...
              const result = TeamModification.importDeck(teamDecks, 2, sourceDeck, "new-uuid");
         ...
         it('should return error for invalid slot index', () => {
              const result = TeamModification.importDeck(teamDecks, 50, INITIAL_DECK, "id");
              expect(result.success).toBe(false);
