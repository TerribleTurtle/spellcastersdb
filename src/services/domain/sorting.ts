  ...
  return (a, b) => {
    switch (mode) {
      ...
      case "Magic School":
        ...
        return compareByCategoryPriority(a, b) || compareByRank(a, b) || compareByName(a, b);
 ...
 */
export function groupItems(items: BrowserItem[], mode: GroupMode): GroupedContent[] {
    ...
    if (mode === "All") {
        ...
        catOrder.forEach((catSingular, index) => {
         ...
         RANKS.forEach((rank, index) => {
             ...
             if (groupItems.length > 0) {
                 groupItems.sort(itemComparator);
                 groups.push({ title: `Rank ${rank}`, items: groupItems, sortIndex: index });
             }
         });
