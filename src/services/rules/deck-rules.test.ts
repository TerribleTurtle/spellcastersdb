...
import { Spellcaster, Unit, Titan } from "@/types/api";
...
const EmptySlot: DeckSlot = { index: 0, unit: null, allowedTypes: [SlotType.Unit] };
const TitanSlot: DeckSlot = { index: 4, unit: null, allowedTypes: [SlotType.Titan] };
...
describe("DeckRules", () => {
    describe("setSpellcaster", () => {
        ...
        it("should not overwrite an existing custom name", () => {
            const deck = DeckRules.setSpellcaster({ ...getInitialDeck(), name: "My Custom Deck" }, MockSpellcaster);
    ...
    describe("quickAdd", () => {
        ...
        it("should fail if the deck is full (units)", () => {
            ...
            const result = DeckRules.quickAdd(deck, { ...MockUnit, entity_id: "u-5" });
