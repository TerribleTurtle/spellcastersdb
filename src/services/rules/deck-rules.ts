...
import { Deck, DeckOperationResult, SlotIndex, SlotType } from "@/types/deck";
...
function addTitan(deck: Deck, entity: Titan): DeckOperationResult<Deck> {
  ...
  const titanSlotIndex = newDeck.slots.findIndex((s) =>
    s.allowedTypes.includes(SlotType.Titan)
  );
  ...
  return {
    success: false,
    error: DECK_ERRORS.NO_TITAN_SLOT,
    code: "NO_TITAN_SLOT",
  };
}
...
function addUnit(deck: Deck, entity: Unit | Spell): DeckOperationResult<Deck> {
  ...
  // Check Duplicates
  const isDuplicate = newDeck.slots.some(
    (s, i) => i < MAX_UNIT_SLOTS && s.unit?.entity_id === entity.entity_id
  );
  if (isDuplicate) {
    return {
      success: false,
      error: DECK_ERRORS.DUPLICATE_UNIT,
      code: "DUPLICATE_UNIT",
    };
  }

  const emptyIndex = newDeck.slots.findIndex(
    (s) => s.index < MAX_UNIT_SLOTS && !s.unit
  );
  if (emptyIndex !== -1) {
...
export const DeckRules = {
  ...
  setSlot(
    ...
    if (isSpellcaster(card)) {
      return {
        success: false,
        error: DECK_ERRORS.SPELLCASTER_IN_NORMAL_SLOT,
        code: "INVALID_TYPE",
      };
    }
    ...
    if (slot.allowedTypes.includes(SlotType.Titan) && !isTitan) {
      return {
        success: false,
        error: DECK_ERRORS.TITAN_SLOT_MISMATCH,
        code: "SLOT_MISMATCH",
      };
    }
    if (slot.allowedTypes.includes(SlotType.Unit) && isTitan) {
      return {
        success: false,
        error: DECK_ERRORS.UNIT_SLOT_MISMATCH,
        code: "SLOT_MISMATCH",
      };
    }
  ...
  swapSlots(
    ...
    if (unitB) {
      ...
      if (error)
        return {
          success: false,
          error: `Taking ${error.toLowerCase()}`,
          code: "SWAP_INVALID",
        };
    }
    ...
    if (unitA) {
      ...
      if (error)
        return {
          success: false,
          error: `Target ${error.toLowerCase()}`,
          code: "SWAP_INVALID",
        };
    }
