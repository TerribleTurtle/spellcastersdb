...
describe("DeckStore", () => {
  ...
  describe("Solo Actions", () => {
    ...
    it("should load deck", () => {
      const { setDeckName, saveDeck, loadDeck, clearDeck } =
  ...
  describe("UI Actions", () => {
    it("should reset isImporting when closing Command Center", () => {
      const { setIsImporting, openCommandCenter, closeCommandCenter } =
        useDeckStore.getState();
  ...
  describe("Persistence Actions", () => {
    it("should check if deck name is available", () => {
      const { saveDeck, setDeckName, checkDeckNameAvailable } =
        useDeckStore.getState();
    ...
    it("should handle duplicate naming collisions", () => {
      ...
      const firstCopy = useDeckStore
        .getState()
        .savedDecks.find((d) => d.name === "My Deck (Copy)");
      expect(firstCopy).toBeDefined();

      // Second duplication -> Should be "My Deck (Copy 1)" or similar, currently fails
      duplicateDeck(originalId);

      const copies = useDeckStore
        .getState()
        .savedDecks.filter((d) => (d.name || "").startsWith("My Deck (Copy"));
