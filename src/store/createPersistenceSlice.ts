...
> = (set, get) => ({
  ...
  saveAsCopy: (nameInput) =>
    set((state) => {
      ...
      const baseName =
        nameInput?.trim() || state.currentDeck.name || "Untitled Deck";
  ...
  upsertSavedTeam: (team) =>
    set((state) => ({
      savedTeams: TeamPersistenceHelper.updateSavedTeams(
        state.savedTeams,
        team
      ),
    })),
