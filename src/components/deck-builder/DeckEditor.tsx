...
import { STORAGE_KEY_SAVED, useDeckBuilder } from "@/hooks/useDeckBuilder";
...
export function DeckEditor({
  ...
  const [selectedItem, setSelectedItem] = useState<
    Unit | Spellcaster | Spell | Titan | null
  >(null);
  ...
  */
  ...
  const handleDragStart = (event: DragStartEvent) => {
    ...
    if (
      current?.item &&
      ("spellcaster_id" in current.item || "entity_id" in current.item)
    ) {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (active.data.current?.type === "slot") {
      ...
      if (
        over &&
        (over.id === "spellcaster-zone" || over.id === "spellcaster-zone-forge")
      ) {
        return;
      }
    ...
    if ("spellcaster_id" in item) {
      if (
        over.id === "spellcaster-zone" ||
        over.id === "spellcaster-zone-forge"
      ) {
        setSpellcaster(item as Spellcaster);
      }
      return;
        ...
        <div className="md:hidden flex bg-surface-main border-b border-white/10 shrink-0">
          {[
            ...
            <button
              key={tab.id}
              onClick={() =>
                handleSwitchTab(tab.id as "BROWSER" | "INSPECTOR" | "FORGE")
              }
              className={cn(
        ...
        <div className="row-span-1 min-h-0 flex-1 flex flex-col md:grid md:grid-cols-12 xl:grid-cols-16 border-b border-white/10 bg-surface-main overflow-hidden">
          ...
          <div
            className={cn(
              ...
              activeMobileTab === "BROWSER" || activeDragItem
                ? "block"
                : "hidden md:block"
            )}
          >
                ...
                {!isTeamMode ? (
                  <div className="w-full flex flex-col gap-2 p-3 bg-brand-primary/10 border border-brand-primary/20 rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button
                        disabled={
                          !deck.spellcaster && deck.slots.every((s) => !s.unit)
                        }
                        onClick={() => {
                          setViewSummary(true);
                        }}
                  ...
                  <div className="w-full flex flex-col gap-2 p-3 bg-brand-primary/10 border border-brand-primary/20 rounded-lg">
                    ...
                    <div className="flex gap-2">
                      {[0, 1, 2].map((idx) => {
                        ...
                        const slotDeck = isActive
                          ? deck
                          : teamDecks
                            ? teamDecks[idx]
                            : null;
                        const validation = slotDeck
                          ? validateDeck(slotDeck)
                          : null;
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="bg-surface-card border border-brand-primary/50 rounded-lg p-6 max-w-lg w-full shadow-2xl relative animate-in fade-in zoom-in duration-200">
              ...
              <p className="text-sm text-gray-400 leading-relaxed mb-6">
                You have an active deck in your workspace. Would you like to
                save it before loading the shared deck?
              </p>
