...
export function useDeckBuilder() {
  ...
  const quickAdd = (item: UnifiedEntity) => {
      if (item.category === EntityCategory.Consumable) return "Consumables cannot be added to decks.";
      ...
      if (isTeamMode) {
          ...
          if (typeof activeSlot === "number" && activeSlot >= 0) {
              ...
              return team.quickAddToTeam(activeSlot, item as any);
          }
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      return solo.quickAdd(item as any);
  };
