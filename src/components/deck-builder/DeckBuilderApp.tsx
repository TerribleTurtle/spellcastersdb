...
export function DeckBuilderApp({ units, spellcasters }: DeckBuilderAppProps) {
  ...
  const activeStorageKey =
    isTeamMode && team.activeSlot !== null
  ...
  useEffect(() => {
    ...
    if (teamHash) {
      ...
      import("@/lib/encoding").then(({ decodeTeam }) => {
        ...
        const importedFingerprints = importedDecks.map((d) =>
          fingerprint(d.spellcasterId, d.slotIds)
        );
        ...
        const currentActiveFingerprints = TEAM_SLOT_KEYS.map((key) => {
          try {
            ...
            return fingerprint(null, [null, null, null, null, null]);
          }
        });

        const isReloadingCurrent =
          JSON.stringify(importedFingerprints) ===
          JSON.stringify(currentActiveFingerprints);
        ...
        if (savedTeamsRaw && importedDecks.length === 3) {
          try {
            const savedTeams = JSON.parse(savedTeamsRaw) as {
              id: string;
              name: string;
              deckData: StoredDeck[];
            }[];
            const found = savedTeams.find((saved) => {
              ...
              const savedFingerprints = saved.deckData.map((d) =>
                fingerprint(d.spellcasterId, d.slotIds)
              );

              return (
                JSON.stringify(importedFingerprints) ===
                JSON.stringify(savedFingerprints)
              );
            });
            ...
            console.error("Error checking saved teams match", e);
          }
        }
        ...
        import("@/hooks/useDeckBuilder").then(({ reconstructDeck }) => {
          ...
          if (savedTeamsRaw) {
            try {
              ...
              const found = savedTeams.find((saved) => {
                if (!saved.deckData || saved.deckData.length !== 3)
                  return false;
                const savedFingerprints = saved.deckData.map((d) =>
                  fingerprint(d.spellcasterId, d.slotIds)
                );
        ...
        if (savedDecksRaw) {
          try {
            ...
            const currentFingerprint = fingerprint(
              newDeck.spellcaster?.spellcaster_id,
              newDeck.slots.map((s) => s.unit?.entity_id)
            );
            ...
            console.error("Error checking saved decks", e);
          }
        }
      }
    }
                ...
                if (viewingTeamData) {
                  if (team.hasChanges) {
                    const confirmed = window.confirm(
                      "You have unsaved changes in your current team. Loading this shared team will overwrite your workspace. Continue?"
                    );
                    if (!confirmed) return;
                  }
