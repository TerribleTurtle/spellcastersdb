...
function VisualSlot({
  ...
  if (isEmpty || !item) {
    return (
      ...
      >
        <span className="text-[10px] font-bold text-gray-600 uppercase tracking-widest text-center px-1">
          {label}
        </span>
      </div>
    );
  ...
  return (
    <div className="flex flex-col items-center gap-3 group/slot">
      ...
      >
        ...
        {/* Badges */}
        {item.category === "Titan" && (
          <div className="absolute top-1 left-1 bg-black/60 px-1.5 py-0.5 rounded text-[10px] font-mono text-brand-accent backdrop-blur-sm shadow-sm border border-brand-accent/20">
            TITAN
          </div>
        )}
        {"rank" in item && item.rank && (
          <div className="absolute top-1 left-1 bg-black/60 px-1.5 py-0.5 rounded text-[10px] font-mono text-gray-300 backdrop-blur-sm border border-white/10">
            {item.rank}
          </div>
        )}
