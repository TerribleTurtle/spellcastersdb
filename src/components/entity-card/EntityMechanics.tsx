...
// Still needed for Shield logic if not kept here
...
export function EntityMechanics({
  item,
  variant = "detailed",
  showDescriptions,
}: EntityMechanicsProps) {
  // Mechanics only exist on Unit or Spell or Ability (legacy)
  const mechanics =
    "mechanics" in item ? (item.mechanics as UnifiedMechanics) : undefined;
  ...
  return (
    <div className={cn("space-y-2", !isCompact && "pt-2")}>
      ...
      {mechanics.pierce && (
        <div
          className={cn(
            "flex items-center gap-1.5 rounded bg-status-info-muted border border-status-info-border p-2 text-status-info-text"
          )}
        >
          <ArrowRight
            size={isCompact ? 13 : 16}
            className="text-status-info-text shrink-0"
          />
      ...
      {mechanics.stealth && (
        <div
          className={cn(
            "flex items-center gap-1.5 rounded bg-surface-dim border border-border-default p-2 text-text-secondary"
          )}
        >
          <Eye
            size={isCompact ? 13 : 16}
          ...
          <div className="flex flex-col">
            <span
              className={cn("font-bold", isCompact ? "text-xs" : "text-sm")}
            >
              Stealth
            </span>
            {shouldShowDescription && (
              <span className="text-[10px] italic opacity-70">
                {mechanics.stealth.duration === -1
                  ? "Infinite"
                  : `${mechanics.stealth.duration}s`}{" "}
                duration
      ...
      {mechanics.cleave && (
        <div
          className={cn(
            "flex items-center gap-1.5 rounded bg-status-danger-muted border border-status-danger-border p-2 text-status-danger-text"
          )}
        >
          <Sword
            size={isCompact ? 13 : 16}
            className="text-status-danger-text/80 shrink-0 rotate-90"
          />
          <div className="flex flex-col">
            <span
              className={cn("font-bold", isCompact ? "text-xs" : "text-sm")}
            ...
            {shouldShowDescription && typeof mechanics.cleave === "object" && (
              <span className="text-[10px] italic opacity-70">
                {mechanics.cleave.arc}° Cone, {mechanics.cleave.radius}m Radius
                (
                {Math.max(0, Math.min(1, 1 - mechanics.cleave.damage_dropoff)) *
                  100}
                % Dmg at edge)
              </span>
            )}
        ...
        modifiers={(() => {
          ...
          return modifiers.filter((mod) => {
            ...
            const allTargetsDuplicated = targets.every((t) => {
              ...
              return (
                bonusValues &&
                Array.from(bonusValues).some(
                  (v) => Math.abs(v - mod.multiplier) < 0.001
                )
              );
            });
      ...
      {mechanics.damage_reduction?.map((mod, i) => (
        ...
        >
          <Shield
            size={isCompact ? 13 : 16}
            className="text-status-success-text shrink-0"
          />
          <div className="flex flex-col">
            <span
              className={cn(
                "text-status-success-text font-bold leading-tight",
                isCompact ? "text-xs md:text-sm" : "text-sm"
              )}
            >
              {((1 - mod.multiplier) * 100).toFixed(1).replace(/\.0$/, "")}%
              Resistance vs{" "}
              <span className="text-text-primary">
                {formatTargetName(mod.source_type)}
              </span>
            </span>
            {mod.condition && (
              <span
                className={cn(
                  "text-green-300/50 italic leading-none",
                  isCompact ? "text-[9px] md:text-[10px]" : "text-xs"
                )}
              >
      ...
      {/* Spawners */}
      <SpawnerList spawners={mechanics.spawner} isCompact={isCompact} />
