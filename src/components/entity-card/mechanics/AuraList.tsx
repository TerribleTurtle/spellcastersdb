      ...
      {auras.map((aura, i) => (
        ...
        >
          ...
          <div className="flex flex-col">
            <span
              className={cn(
                "text-blue-200 font-bold leading-tight",
                isCompact ? "text-xs" : "text-sm"
              )}
            >
              {aura.name || "Aura"}:{" "}
              <span className="text-text-primary text-base">{aura.value}</span>{" "}
              ({aura.effect || "Effect"}){!isCompact && ` / ${aura.interval}s`}
            </span>
            <span
              className={cn(
                "text-blue-300/70",
                isCompact ? "text-[9px] leading-tight" : "text-xs"
              )}
            >
              Target:{" "}
              {aura.target_type
                ? formatTargetName(aura.target_type)
                : aura.target_types?.map(formatTargetName).join(", ") ||
                  "Unknown"}{" "}
              • Radius: {aura.radius}m{" "}
              {isCompact && `• Interval: ${aura.interval}s`}
            </span>
