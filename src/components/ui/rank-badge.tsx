...
import { RANK_STYLES } from "@/services/config/rank-class-styles";
...
export function RankBadge({
  ...
  const finalStyle = isTitan ? RANK_STYLES["V"] : style;

  if (!finalConfig || !finalStyle) return null;

  const label = isTitan ? "TITAN" : finalConfig.label;

  // Icon Mode (Geometric Shapes via SVG)
  if (mode === "icon") {
    const strokeClass = finalStyle.stroke;
    // const fillClass = finalConfig.bg;
    // Wait, previous constants change added `fill` key like `fill-emerald-700`.
    // Let's use `finalStyle.fill` directly.
    const svgFill = finalStyle.fill; // e.g. "fill-emerald-700"
        ...
        case "I": // Circle
          ...
          shape = (
            <circle
              ...
              cy="12"
              r="10.5"
              className={cn("stroke-2", strokeClass, svgFill)}
            />
          );
          break;
        case "II": // Square
