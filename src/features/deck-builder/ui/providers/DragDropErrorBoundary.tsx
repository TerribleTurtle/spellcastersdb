    ...
    if (this.state.hasError) {
      return (
        <div className="h-full w-full flex flex-col items-center justify-center p-8 text-center bg-surface-main">
          <div className="w-16 h-16 rounded-full bg-status-danger-muted flex items-center justify-center mb-4 border border-status-danger-border">
            <AlertTriangle className="text-status-danger" size={32} />
          </div>
          ...
          <button
            ...
            className="px-6 py-2 bg-brand-primary text-brand-dark rounded-lg font-bold uppercase tracking-wider flex items-center gap-2 hover:bg-brand-primary/80 transition-colors"
          >
            <RefreshCw size={16} />
            Reset Interface
          </button>
