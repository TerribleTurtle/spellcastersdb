...
const createMockSlots = () => Array(5).fill(null).map((_, i) => ({
  index: i,
  unit: null,
...
describe('useTeamEditor Drawer Logic', () => {
  ...
  it('should enforce accordion (single open) on Mobile (<1280px)', () => {
    ...
    expect(result.current.accordion.expandedState).toEqual([true, false, false]);
    ...
    expect(result.current.accordion.expandedState).toEqual([false, true, false]);
  });
