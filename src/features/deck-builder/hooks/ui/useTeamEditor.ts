...
export function useTeamEditor() {
  ...
  const { setTeamDecks, setTeamName, openCommandCenter, currentDeck } =
    useDeckStore(
      useShallow((state) => ({
  ...
  const footerHeight = accordion.expandedState.reduce(
    (acc, expanded) =>
      acc + (expanded ? TRAY_EXPANDED_HEIGHT : TRAY_COLLAPSED_HEIGHT),
    0
  );
  ...
  const handleTeamSave = () => {
    ...
    saveTeam(
      ...
      activeSlot !== null ? currentDeck : undefined
    );
