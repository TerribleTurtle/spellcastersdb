...
export const ActiveDeckTray = memo(
  function ActiveDeckTray({
    slots,
    spellcaster,
    onSelect,
    deckId,
  ...
  (prevProps, nextProps) => {
    return (
      ...
      prevProps.prioritySpellcaster === nextProps.prioritySpellcaster &&
      prevProps.spellcaster?.entity_id === nextProps.spellcaster?.entity_id &&
      prevProps.slots.length === nextProps.slots.length &&
      prevProps.slots.every(
        (s, i) => s.unit?.entity_id === nextProps.slots[i].unit?.entity_id
      ) &&
      prevProps.validation?.isValid === nextProps.validation?.isValid &&
      prevProps.validation?.errors?.length ===
        nextProps.validation?.errors?.length
    );
  }
);
