...
export function DeckSlot({
  ...
  const baseDragId = deckId ? `slot-drag-${deckId}-${slot.index}` : `slot-drag-${slot.index}`;
  ...
  if (currentDrag && currentDrag.item) {
      ...
      if ("spellcaster_id" in item || item.category === ENTITY_CATEGORY.Spellcaster) {
          isValidTarget = false;
            ...
            // Singleton Rule
            const isInternalMove = currentDrag.type === "DECK_SLOT" && 
            ...
            const isDuplicate = !isInternalMove && slot.index < 4 && allSlots.some(
                (s, i) => i < 4 && i !== slot.index && s.unit?.entity_id === unitItem.entity_id
            );
