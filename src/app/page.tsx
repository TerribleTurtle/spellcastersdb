    ...
    >
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-10">
        <Link
          ...
          className="group relative overflow-hidden rounded-2xl bg-surface-card border border-border-default p-8 hover:border-brand-primary/50 transition-all hover:bg-surface-raised active:scale-[0.98] animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out fill-mode-both"
        ...
        <Link
          ...
          className="group relative overflow-hidden rounded-2xl bg-surface-card border border-border-default p-8 hover:border-brand-secondary/50 transition-all hover:bg-surface-raised active:scale-[0.98] animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out fill-mode-both"
        ...
        <Link
          ...
          className="group relative overflow-hidden rounded-2xl bg-surface-card border border-border-default p-8 hover:border-brand-accent/50 transition-all hover:bg-surface-raised active:scale-[0.98] animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out fill-mode-both"
      ...
      <div className="grid md:grid-cols-3 gap-8">
        <div className="md:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <Link
              href="/changes"
              className="text-sm text-brand-primary hover:text-brand-accent whitespace-nowrap transition-colors"
            >
              All Changes <ArrowRight size={16} className="ml-1 inline" />
            </Link>
          </div>
          ...
          <div className="bg-surface-card border border-border-default rounded-xl p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h4 className="text-lg font-bold text-text-primary">
                  {latestPatch?.version && latestPatch.version.length > 20
                    ? `Patch — ${
                        latestPatch?.date
                          ? new Date(latestPatch.date).toLocaleDateString(
                              undefined,
                              {
                                year: "numeric",
                                month: "short",
                ...
                <p className="text-sm text-text-muted">
                  {latestPatch?.date
                    ? new Date(latestPatch.date).toLocaleDateString(undefined, {
                        year: "numeric",
                        month: "long",
                        day: "numeric",
                      })
            ...
            <div className="space-y-3 mb-4">
              {(latestPatch ? latestPatch.changes.slice(0, 3) : []).map(
                (change, i) => (
                  <div
                    key={i}
                    className="flex gap-3 text-sm border-l-2 border-border-default pl-3 py-1"
                  >
                    <span
                      className={`font-mono text-xs px-1.5 py-0.5 rounded uppercase flex items-center gap-1 ${
                        change.change_type === "add"
                          ? "bg-status-success-muted text-status-success-text"
                          : change.change_type === "delete"
                            ? "bg-status-danger-muted text-status-danger-text"
            ...
            {!latestPatch && (
              <div className="text-center py-8 text-text-muted bg-surface-dim rounded-lg mb-4">
            ...
            <Link
              href="/changes"
              className="block w-full text-center py-2 rounded-lg bg-surface-card hover:bg-surface-hover border border-border-default text-sm text-text-secondary transition-colors"
            >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-surface-card border border-border-default rounded-xl p-6 space-y-4">
            ...
            <a
              ...
              className="flex items-center justify-center gap-2 w-full py-2.5 rounded-lg bg-discord hover:bg-discord/90 text-text-primary font-medium transition-colors"
            >
              Join Discord
            </a>
            <a
          ...
          <div className="bg-surface-card border border-border-default rounded-xl p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              {stats.map((stat) => (
                <div
                  key={stat.label}
                  className="bg-surface-dim p-3 rounded-lg border border-border-subtle"
                >
